...
import { recentTracksData } from "../temp";
import React from "react";

const Recent: FunctionComponent = () => {

    const [recentlyPlayed, setRecentlyPlayed] = useState<any>(recentTracksData);
    ...
    return (
        <div>
            {recentlyPlayed.length == 0 ? <Loader /> : <div className="m-auto w-full px-24 my-16 text-white">
                    ...
                    {recentlyPlayed && recentlyPlayed.items.map((recent: any, i: number) => (
                        <div key={i} className="w-[100%] flex items-center justify-between">
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    <p className="text-base font-semibold mt-2">{recent.track.name}</p>
