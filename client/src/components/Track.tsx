    ...
    return (
        <div className="w-[100%] flex items-center justify-between">
            <div className="flex items-center lg:gap-5 md:gap-4 gap-3">
                <Link to={`/track/${trackId}`} className='h-16 w-16 bg-gray-900'>
                    <img className="h-16 w-16" src={trackImage} alt={trackName} />
                </Link>

                <div>
                    <Link to={`/track/${trackId}`}><p className="text-base font-semibold mt-2">{trackName}</p></Link>
                    ...
                    {/* &nbsp;·&nbsp;  */}
                    <Link className="hover:underline hover:text-green-500" to={`/album/${tractAlbumId}`}>
